 ...
 * Muestra hero, estadísticas reales y coreografías destacadas / más vendidas.
 */
...
export default function LandingPage() {
  ...
  const [hotSales, setHotSales] = useState<Choreography[]>([]);
  ...
  const ChoreoCard = ({ c }: { c: Choreography }) => (
    <Link
      to="/catalog"
    ...
    <div className="min-h-screen bg-[#1A1A1A]">
      ...
      <section className="pt-32 pb-16 px-6 text-center max-w-7xl mx-auto">
        ...
        <div className="flex justify-center gap-12 py-8 border-t border-b border-[#333] max-w-2xl mx-auto">
          <div>
            <span className="font-display text-4xl text-[#FF6B1A]">{stats?.choreographies_count ?? '—'}+</span>
            <p className="text-sm text-gray-400 mt-1">Coreografías</p>
          </div>
          <div>
            <span className="font-display text-4xl text-[#FF6B1A]">{stats?.professors_count ?? '—'}</span>
            <p className="text-sm text-gray-400 mt-1">Profesores</p>
          </div>
          <div className="flex items-baseline gap-1">
            <span className="font-display text-4xl text-[#FF6B1A]">{stats?.average_rating ?? '—'}</span>
      ...
      <section className="px-6 pb-12 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {featured.map((c) => <ChoreoCard key={c.id} c={c} />)}
        </div>
      </section>

      {hotSales.length > 0 && (
        <section className="px-6 pb-20 max-w-7xl mx-auto">
          <h2 className="font-display text-3xl tracking-wide mb-8">MÁS VENDIDAS</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {hotSales.map((c) => <ChoreoCard key={c.id} c={c} />)}
          </div>
        </section>
      )}
